...
void ClientOSRHandler::OnPaint(CefRefPtr<CefBrowser> browser,
    ...
    if ([syphonServer hasClients])
    {
        ...
        NSLog(@"OnPaint called, no syphon clients connected.");
    }
...
CefRefPtr<OSRWindow> OSRWindow::Create(OSRBrowserProvider* browser_provider,
                                       ...
                                       const CefRect& frame,
                                       NSString *syphonName) {
    return new OSRWindow(browser_provider, transparent, parentView, frame, syphonName);
}
...
OSRWindow::OSRWindow(OSRBrowserProvider* browser_provider,
    ...
    this->render_client = new ClientOSRHandler(view, browser_provider, syphonName);
}
